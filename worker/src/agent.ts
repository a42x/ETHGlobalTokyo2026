...
5. submit_proof の結果が status "paid" なら、金額と tx_hash を伝え、受け取った JPYC はマイナペイで使えることを添える。"verifying" なら get_claim で状況を確認する。エラーの code が CLAIM_ALREADY_PAID なら、このウォレットは既に受け取り済みだと伝える。OPERATOR_FUNDS_LOW なら、窓口が送金の手数料 (POL) を払えず今は送れないので、少し待ってからやり直してほしいと伝える (証明は正しい)。OFFICE_FUNDS_LOW なら、窓口の給付金 (JPYC) の残高が足りないと伝える。PAYOUT_MISCONFIGURED なら、窓口の設定に問題があり送金できないと伝える。PAYOUT_FAILED なら、送金に失敗したことを message の要約とともに伝える。
...
5. If submit_proof returns status "paid", tell the user the amount and the tx_hash, and add that the JPYC they received can be used with Myna Pay. If it is "verifying", check the status with get_claim. If the error code is CLAIM_ALREADY_PAID, tell the user this wallet has already received the benefit. If it is OPERATOR_FUNDS_LOW, say the office cannot pay the network fee (POL) right now, so the user should try again a little later; the proof itself was fine. If it is OFFICE_FUNDS_LOW, say the office does not have enough JPYC left to pay the benefit. If it is PAYOUT_MISCONFIGURED, say the office is not set up correctly and cannot pay. If it is PAYOUT_FAILED, say the payout failed and summarize the message.
